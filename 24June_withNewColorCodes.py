...
class Ui_Dialog(object):
    ...
    def summaryReport(self):
        ...
        final_result_summ2['PercentileInc'] = final_result_summ2['Inconsistent']/77 * 100
        final_result_summ2['PercentilePRErr'] = final_result_summ2['Probable Reporting Error']/ 77 * 100
        
        len0 = len(final_result_summ2['PercentileInc'])
        len1 = len(final_result_summ2['PercentilePRErr'])
        ...
        col_percentileInc_rank = []
        col_percentileErr_rank = []

        for k in range(0,len0):
            primcol_Inc = final_result_summ2['PercentileInc'][k]
            if primcol_Inc < 5 :
                col_percentileInc_rank.append('1')
            elif primcol_Inc >= 5 and primcol_Inc < 10:
                col_percentileInc_rank.append('2')
            elif primcol_Inc >= 10 and primcol_Inc < 25:
                col_percentileInc_rank.append('3')
            elif primcol_Inc >= 25 and primcol_Inc < 50:
                col_percentileInc_rank.append('4')
            elif primcol_Inc >= 50:
                col_percentileInc_rank.append('5')
            else:
                col_percentileInc_rank.append('NaN')

        for i in range(0,len1):
            primcol_Prc = final_result_summ2['PercentilePRErr'][i]
            if primcol_Prc < 5 :
                col_percentileErr_rank.append('1')
            elif primcol_Prc >= 5 and primcol_Prc < 10:
                col_percentileErr_rank.append('2')
            elif primcol_Prc >= 10 and primcol_Prc < 25:
                col_percentileErr_rank.append('3')
            elif primcol_Prc >= 25 and primcol_Prc < 50:
                col_percentileErr_rank.append('4')
            elif primcol_Prc >= 50:
                col_percentileErr_rank.append('5')
            else:
                col_percentileErr_rank.append('NaN')

        # sum of columns
        col_sum = []

        for j in range(0,len0):
            num1 = int(col_percentileInc_rank[j])
            num2 = int(col_percentileErr_rank[j])
            sum = num1 + num2
            col_sum.append(sum)
        ...
        del final_result_summ2['PercentileInc']
        del final_result_summ2['PercentilePRErr']


        def select_col(x):
            # COLORS
            c = ['background-color:  #cc3333',              #>=50% RED
                    'background-color: #F9A825',            #25-50% ORANGE
                        'background-color: #FFFF8D',        #10-25% YELLOW
                            'background-color: #AED581',    #5-10% L GREEN
                                'background-color: #689F38',#<5% GREEN
                                    '']
