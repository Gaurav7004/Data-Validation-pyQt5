...
class Ui_Dialog(object):
    ...
    def summaryReport(self):
        ...
        # print(len(col_percentageInc_Bucket))
        ...
        # del final_result_summ2['Inconsistent']
        # del final_result_summ2['Probable Reporting Error']
        del final_result_summ2['PercentageInc']
        del final_result_summ2['PercentagePRErr']

        new_order = [0, 1, 5, 2, 6, 4, 3]
        final_result_summ2[final_result_summ2.columns[new_order]]

        def select_col(X):
            global cnt1, cnt2, cnt3, cnt4, cnt5
            # COLORS
            ...
            # df1.loc[mask_4, 'Inconsistent'] = c[4]
            ...
            df1.loc[mask_10, 'Score'] = c[0]
            df1.loc[mask_11, 'Score'] = c[1]
            df1.loc[mask_12, 'Score'] = c[2]
            df1.loc[mask_13, 'Score'] = c[3]
            df1.loc[mask_14, 'Score'] = c[4]
        ...
        for k in range(0,len3):
            ...
            if primcol_Con < 5 :
                col_percentageCon_Bucket.append('<0.05')
            elif primcol_Con >= 5 and primcol_Con < 10:
                col_percentageCon_Bucket.append('>= 0.05 & < 0.10')
            elif primcol_Con >= 10 and primcol_Con < 25:
                col_percentageCon_Bucket.append('>= 0.10 & < 0.25')
            elif primcol_Con >= 25 and primcol_Con < 50:
                col_percentageCon_Bucket.append('>= 0.25 & < 0.5')
            elif primcol_Con >= 50:
                col_percentageCon_Bucket.append('>= 0.5')
            else:
        ...
        def select_col_Con(X):
            ...
            mask_15 = X['PercentageCon'] >= 0.50
            mask_16 = (X['PercentageCon'] < 0.50) & (X['PercentageCon'] >= 0.25)
            mask_17 = (X['PercentageCon'] < 0.25) & (X['PercentageCon'] >= 0.10)
            mask_18 = (X['PercentageCon'] < 0.10) & (X['PercentageCon'] >= 0.05)
            mask_19 = X['PercentageCon'] < 0.05          
    ...
    def export(self):
        ...
        sheet = workbook['Summary Report-2']
        ...
        ele = pd.DataFrame({
            ...
            'Scores': [cnt1, cnt2, cnt3, cnt4, cnt5]
        })

        # # plotting a bar graph
        # ele.plot(x="Conditions", y="Scores", kind="bar")

        # plt.title('Frequency')
        # plt.savefig("myplot.png", dpi = 150)

        # img = openpyxl.drawing.image.Image('myplot.png')
        # img.anchor(workbook.cell('O3'))
        # sheet.add_image(img)
        ...
        # PALETTES
        workbook = load_workbook(filename)
        sheet = workbook['Summary Report-3']
        workbook.active = sheet
        sheet["K10"] = "< 5%"
        sheet["L10"] = "VERY POOR"
        sheet["K11"] = "5 - 10%"
        ...
        sheet['L10'].fill = PatternFill(fgColor="00AF5F", fill_type = "solid")
        sheet['L11'].fill = PatternFill(fgColor="00FF00", fill_type = "solid")
        sheet['L12'].fill = PatternFill(fgColor="C0C000", fill_type = "solid")
        sheet['L13'].fill = PatternFill(fgColor="FFAF00", fill_type = "solid")
        sheet['L14'].fill = PatternFill(fgColor="C00000", fill_type = "solid")
        workbook.save(filename=filename)
