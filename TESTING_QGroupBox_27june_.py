...
class Ui_Dialog(object):
    ...
    def DH_Validate(self):
        ...
        def res48(df):
            if pd.isnull(df['col_193']) and pd.isnull(df['col_194']) and pd.isnull(df['col_184']) and pd.isnull(df['col_185']) and pd.isnull(df['col_186']) and pd.isnull(df['col_187']) and pd.isnull(df['col_188']) and pd.isnull(df['col_189']) and pd.isnull(df['col_190']) and pd.isnull(df['col_191']) and pd.isnull(df['col_192']):
                ...
                if pd.isnull((float(df['col_193'])) + (float(df['col_194']))) and not pd.isnull(float(df['col_184']) + float(df['col_185']) + float(df['col_186'])
                                                                                                + float(df['col_187']) + float(df['col_188']) + float(df['col_189'])
                                                                                                + float(df['col_190']) + float(df['col_191']) + float(df['col_192'])):
                    return 'Inconsistent'
                elif not pd.isnull((float(df['col_193'])) + (float(df['col_194']))) and pd.isnull(float(df['col_184']) + float(df['col_185']) + float(df['col_186'])
                                                                                                + float(df['col_187']) + float(df['col_188']) + float(df['col_189'])
                                                                                                + float(df['col_190']) + float(df['col_191']) + float(df['col_192'])):
                    return 'Probable Reporting Error'
            elif (float(df['col_193']) + float(df['col_194'])) > (float(df['col_184']) + float(df['col_185']) + float(df['col_186'])
                                                                                                + float(df['col_187']) + float(df['col_188']) + float(df['col_189'])
                                                                                                + float(df['col_190']) + float(df['col_191']) + float(df['col_192'])):
                return 'Inconsistent'
            else:
    ...
    def SDH_Validate(self):
        ...
        def res48(df):
            if pd.isnull(df['col_194']) and pd.isnull(df['col_195']) and pd.isnull(df['col_185']) and pd.isnull(df['col_186']) and pd.isnull(df['col_187']) and pd.isnull(df['col_188']) and pd.isnull(df['col_189']) and pd.isnull(df['col_190']) and pd.isnull(df['col_191']) and pd.isnull(df['col_192'])and pd.isnull(df['col_193']):
                ...
                if pd.isnull((float(df['col_194'])) + (float(df['col_195']))) and not pd.isnull(float(df['col_185']) + float(df['col_186']) + float(df['col_187'])
                                                                                                + float(df['col_188']) + float(df['col_189']) + float(df['col_190'])
                                                                                                + float(df['col_191']) + float(df['col_192']) + float(df['col_193'])):
                    return 'Inconsistent'
                elif not pd.isnull((float(df['col_194'])) + (float(df['col_195']))) and pd.isnull(float(df['col_185']) + float(df['col_186']) + float(df['col_187'])
                                                                                                + float(df['col_188']) + float(df['col_189']) + float(df['col_190'])
                                                                                                + float(df['col_191']) + float(df['col_192']) + float(df['col_193'])):
                    return 'Probable Reporting Error'
    ...
    def CHC_Validate(self):
        ...
        def res47(df):
            if pd.isnull(df['col_192']) and pd.isnull(df['col_193']) and pd.isnull(df['col_183']) and pd.isnull(df['col_184']) and pd.isnull(df['col_185']) and pd.isnull(df['col_186']) and pd.isnull(df['col_187']) and pd.isnull(df['col_188']) and pd.isnull(df['col_189']) and pd.isnull(df['col_190']) and pd.isnull(df['col_191']):
                ...
                if pd.isnull((float(df['col_192'])) + (float(df['col_193']))) and not pd.isnull(float(df['col_183']) + float(df['col_184']) + float(df['col_185'])
                                                                                                + float(df['col_186']) + float(df['col_187']) + float(df['col_188'])
                                                                                                + float(df['col_189']) + float(df['col_190']) + float(df['col_191'])):
                    return 'Inconsistent'
                elif not pd.isnull((float(df['col_192'])) + (float(df['col_193']))) and pd.isnull(float(df['col_183']) + float(df['col_184']) + float(df['col_185'])
                                                                                                + float(df['col_186']) + float(df['col_187']) + float(df['col_188'])
                                                                                                + float(df['col_189']) + float(df['col_190']) + float(df['col_191'])):
                    return 'Probable Reporting Error'
    ...
    def summaryReport(self):
        global final_result_summ1, final_result_summ2, col_sum
        ...
        def select_col_SumSheet(X):
            ...
            # Coloring percentage >=50% to RED
            count_rows = df.shape[0]

            mask_21 = X['Inconsistent']/count_rows >= 0.5
            cnt21 = mask_21.values.sum()    

            mask_22 = X['Probable Reporting Error']/count_rows >= 0.5
        ...
        # For Primary Health Centre
            ...
            final_result_summ2['PercentageInc'] = final_result_summ2['Inconsistent']/77 * 100
            final_result_summ2['PercentagePRErr'] = final_result_summ2['Probable Reporting Error']/ 77 * 100
    ...
    def export(self):
        ...
        sheet["K21"] = "5 - 10%"
        sheet["K22"] = "< 5%"
        sheet["K23"] = "Total Facilities"

        sheet["L17"] = "Inconsistent"
        sheet["L18"] = cnt1
        sheet["L19"] = cnt2
        sheet["L20"] = cnt3
        sheet["L21"] = cnt4
        sheet["L22"] = cnt5
        sheet["L23"] = cnt1 + cnt2 + cnt3 + cnt4 + cnt5

        sheet["M17"] = "Probable Reporting Error"
        sheet["M18"] = cnt6
        sheet["M19"] = cnt7
        sheet["M20"] = cnt8
        sheet["M21"] = cnt9
        sheet["M22"] = cnt10
        sheet["M23"] = cnt6 + cnt7 + cnt8 + cnt9 + cnt10

        sheet['L10'].fill = PatternFill(fgColor="C00000", fill_type = "solid")
        sheet['L11'].fill = PatternFill(fgColor="FFAF00", fill_type = "solid")
        sheet['L12'].fill = PatternFill(fgColor="C0C000", fill_type = "solid")
        sheet['L13'].fill = PatternFill(fgColor="00FF00", fill_type = "solid")
        sheet['L14'].fill = PatternFill(fgColor="00AF5F", fill_type = "solid")

        
        # from matplotlib.pyplot import figure
        # ele = pd.DataFrame({
        #     'Scores': ['1', '2' , '3', '4', '5'],
        #     'Numbers': [cnt5, cnt4, cnt3, cnt2, cnt1]
        # })

        # # plotting a bar graph
        # ele.plot(x="Scores", y="Numbers", kind="bar")
        # # figure(figsize=(8, 8), dpi=80)

        # plt.title('Scores wrt Inconsistent & PRE')
        ...
        # # PALETTES
        # workbook = load_workbook(filename)
        # sheet = workbook['Facility Guidance Sheet_2']
        # workbook.active = sheet
        # sheet["H10"] = "< 5%"
        # sheet["I10"] = "VERY POOR"
        # sheet["H11"] = "5 - 10%"
        ...
        # sheet['I10'].fill = PatternFill(fgColor="C00000", fill_type = "solid")
        # sheet['I11'].fill = PatternFill(fgColor="FFAF00", fill_type = "solid")
        # sheet['I12'].fill = PatternFill(fgColor="C0C000", fill_type = "solid")
        # sheet['I13'].fill = PatternFill(fgColor="00FF00", fill_type = "solid")
        # sheet['I14'].fill = PatternFill(fgColor="00AF5F", fill_type = "solid")

        
        # ele = pd.DataFrame({
        #     'Conditions': ['>= 50%' , '25 - 50%', '10 - 25%', '5 - 10%', '< 5%'],
        #     'Count': [cnt11, cnt12, cnt13, cnt14, cnt15]
        # })

        # # plotting a bar graph
        # ele.plot(x="Conditions", y="Count", kind="bar")
        # # figure(figsize=(8, 8), dpi=80)

        # plt.title('Count for Consistent(Percentage wise)')
        # plt.savefig("myplot_1.png", dpi = 150)

        # img = openpyxl.drawing.image.Image('myplot_1.png')
        # img.anchor='L3'
